...
contract Media is IMedia, ERC721Burnable, ReentrancyGuard {
    ...
    function _mintForCreator(
        ...
        MediaData memory data,
        IMarket.BidShares memory bidShares
    ) internal onlyValidURI(data.tokenURI) onlyValidURI(data.metadataURI) //returns (string memory, uint256 memory)
    {
        ...
        EnumerableSet.UintSet storage set1 = _creatorTokens[creator];
        EnumerableSet.add(set1,tokenId);
